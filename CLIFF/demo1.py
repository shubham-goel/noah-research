...
def main(args):
    ...
    my_cliff_model = CLIFFWrapper2023(args.ckpt)
    my_cliff_model = my_cliff_model.to(device)
    ...
    for batch in tqdm(mocap_data_loader):
        ...
        mybatch = {
            ...
            'box_center': center,
            'box_size': scale * 200,
            'img_size': torch.stack((img_w, img_h), dim=-1),
        }
        myout, _ = my_cliff_model(mybatch, input_is_square=False)
        pred_cam_crop = myout['pred_cam']
        pred_rotmat = torch.cat((
            myout['pred_smpl_params']['global_orient'],
            myout['pred_smpl_params']['body_pose']), dim=1)
        pred_betas = myout['pred_smpl_params']['betas']
